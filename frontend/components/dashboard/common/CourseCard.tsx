...
const CourseCard: React.FC<ReusableCourseCardProps> = ({
  ...
  const progressValue = Math.max(0, Math.min(100, course.progressPercent ?? 0));
  const actionLabel = course.actionLabel || "Start Courses";
  const secondaryActionLabel = course.secondaryActionLabel || "Review Courses";
  ...
  return (
    <div className='w-full max-w-70 rounded-[10px] border border-(--color-card-border) bg-(--color-card-bg) p-3.5 shadow-[0_2px_10px_rgba(15,23,42,0.06)]'>
      ...
      {hasProgress && (
        <div className='mt-3'>
          <div className='mb-1 flex items-center justify-between'>
            <span className='text-xs text-(--color-text-primary)'>Progress</span>
            <span className='text-xs text-(--color-placeholder-text)'>
              {progressValue}%
            </span>
          </div>
